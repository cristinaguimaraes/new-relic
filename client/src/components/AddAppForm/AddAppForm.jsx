...
import {Helper, Input, Label, Form, Button, ButtonWrapper, Title} from './styled';
...
const AddAppForm  =({onAddApp, onShowToastMessage}) => {
  ...
  const handleOnSubmit = (event) => {
    ...
    const errors = {};
    if (!app.name)
      errors.name = 'Please add the name';
  ...
  if (Object.keys(errors).length === 0) {
    ...
    return (
        ...
        <Form onSubmit={handleOnSubmit} >
          ...
          <Input name='name' type='text' value={app.name} onChange={handleOnChange} />
          {errors.name && <Helper>{errors.name}</Helper>}
          <Label>Contributors:</Label>
          <Input name='contributors' type='text' value={app.contributors.toString()} onChange={handleOnChange} />
          {errors.contributors && <Helper>{errors.contributors}</Helper>}
          <Label>Apdex:</Label>
          <Input name='apdex' type='text' value={app.apdex} onChange={handleOnChange} />
          {errors.apdex && <Helper>{errors.apdex}</Helper>}
          <Label>Version:</Label>
          <Input name='version' type='text' value={app.version} onChange={handleOnChange} />
          {errors.version && <Helper>{errors.version}</Helper>}
          <Label>Host:</Label>
          <Input name='host' type='text' value={app.host.toString()} onChange={handleOnChange} />
